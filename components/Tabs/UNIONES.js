...
class ComponentList extends React.PureComponent {
    ...
    render() {
        return (
            <FlatList
                data={this.props.UNIONEScomponentsFounds}
...
export default function UNIONES({ navigation }) {

    const templateContext = useContext(TemplateContext);
    const { components, UNIONEScomponentsFounds, coordXY, setcoordXY, searchComponentUNIONES, translateX, translateY } = templateContext;
    ...
    const handleChange = e => {
        ...
        if (searchText === '') {
            ...
            searchComponentUNIONES(searchText);
        }
    };
    ...
    const handleAddComponent = useMemo(
        () => (idx) => {
            ...
            let selectedComponent = UNIONEScomponentsFounds.filter((item) => item.id === idx);
    ...
    return (
        <SafeAreaView style={{ flex: 1, justifyContent: "center", backgroundColor: "white" }}>
            ...
            <ComponentList UNIONEScomponentsFounds={UNIONEScomponentsFounds} handleAddComponent={handleAddComponent} />
        </SafeAreaView>
    );
}
