...
class ComponentList extends React.PureComponent {
    ...
    render() {
        return (
            <FlatList
                data={this.props.IECcomponentsFounds}
...
export default function IEC({ navigation }) {

    const templateContext = useContext(TemplateContext);
    const { IECcomponents, IECcomponentsFounds, coordXY, setcoordXY, searchComponentIEC, translateX, translateY } = templateContext;
    ...
    const handleAddComponent = useMemo(
        ...
        }, [IECcomponentsFounds, navigation, setcoordXY]);
    return (
        <SafeAreaView style={{ flex: 1, justifyContent: "center", backgroundColor: "white" }}>
            ...
            <ComponentList IECcomponentsFounds={IECcomponentsFounds} handleAddComponent={handleAddComponent} />
        </SafeAreaView>
    );
}
